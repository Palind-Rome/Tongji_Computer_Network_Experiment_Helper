...
TOOLS: list[dict] = [
    ...
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "slot": {
                    "type": "string",
                    "description": "Slot identifier, e.g. 0/0, 0/1, or 0/0/0 depending on device/model.",
                },
                "model": {
                    "type": "string",
    ...
    },
    {
        "name": "sendPdu",
        "description": (
            "Test ICMP reachability between two devices. On newer PT versions this uses "
            "the native Simple PDU mechanism; on PT6 it falls back to running a ping "
            "command from the source device to the destination device's IPv4 address."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        "name": "debugEval",
        "description": (
            "PT6 debugging helper: evaluate a JavaScript snippet inside the Packet "
            "Tracer script engine and return the result. Use only for trusted local "
            "diagnostics and compatibility fixes."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "string",
                    "description": "JavaScript body to run, for example: return getNetwork();",
                },
            },
            "required": ["code"],
        },
